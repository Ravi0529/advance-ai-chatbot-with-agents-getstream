...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
    ...
    const handleStopGenerating = () => {
      if (channel) {
        const aiMessage = messages
          ? [...messages].reverse().find((m) => m.user?.id.startsWith("ai-bot"))
          : undefined;
        if (aiMessage) {
          channel.sendEvent({
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      <header className="shrink-0 flex items-center justify-between px-4 py-3 border-b bg-background/95 backdrop-blur-sm z-10">
